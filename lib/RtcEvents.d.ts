...
declare type UidWithMutedCallback = (uid: number, muted: boolean) => void;
declare type VideoSizeCallback = (uid: number, width: number, height: number, rotation: number) => void;
declare type RemoteVideoStateCallback = (uid: number, state: VideoRemoteState, reason: VideoRemoteStateReason, elapsed: number) => void;
declare type LocalVideoStateCallback = (localVideoState: LocalVideoStreamState, error: LocalVideoStreamError) => void;
declare type RemoteAudioStateCallback = (uid: number, state: AudioRemoteState, reason: AudioRemoteStateReason, elapsed: number) => void;
...
declare type AudioMixingStateCallback = (state: AudioMixingStateCode, errorCode: AudioMixingErrorCode) => void;
...
declare type VideoFrameWithUidCallback = (uid: number, width: number, height: number, elapsed: number) => void;
declare type UrlWithErrorCallback = (url: string, error: ErrorCode) => void;
declare type UrlCallback = (url: string) => void;
declare type TransportStatsCallback = (uid: number, delay: number, lost: number, rxKBitRate: number) => void;
declare type UidWithEnabledCallback = (uid: number, enabled: boolean) => void;
declare type EnabledCallback = (enabled: boolean) => void;
declare type AudioQualityCallback = (uid: number, quality: number, delay: number, lost: number) => void;
declare type MetadataCallback = (buffer: string, uid: number, timeStampMs: number) => void;
declare type FacePositionCallback = (imageWidth: number, imageHeight: number, faces: [FacePositionInfo]) => void;
/**
